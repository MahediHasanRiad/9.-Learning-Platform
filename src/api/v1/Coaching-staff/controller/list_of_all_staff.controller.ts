...
export const allCoachingStaffController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const filterByCoaching = await FilterStaffBySearch({
      coachingId,
      role,
      search,
      sortType,
    ...
    const staff = filterByCoaching?.map((item) => ({
      ...item,
      staffLink: `/users/${item.id}`,
    }));

    // pagination
    const totalItems = await prisma.coachingStaff.count({
      where: {
        coachingId: coachingId!,
      },
    });
    const pagination = Pagination(page, limit, totalItems, `coachingStaffs`);

    // links
    const links = Links(req, pagination, page, `coachingStaffs`);
